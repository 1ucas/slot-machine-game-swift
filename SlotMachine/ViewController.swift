...
//  SlotMachine
//
//  Created by Lucas Maciel on 7/13/15.
...
class ViewController: UIViewController {
    ...
    func betOneButtonPressed (button: UIButton) {
        if credits <= 0 {
            showAlertWithText("No More Credits", messsage: "Reset Game")
        }
        else {
            if currentBet < 5{
                currentBet += 1
    ...
    func betMaxButtonPressed (button: UIButton) {
        if credits <= 5 {
            showAlertWithText("Not enough credits", messsage: "Bet Less")
        }
        else {
            if currentBet < 5 {
    ...
    func spinButtonPressed (button: UIButton) {
        ...
        let winningMultiplier = SlotBrain.computeWinnings(slots)
    ...
    func showAlertWithText (header: String = "Warning", messsage:String){
        let alert = UIAlertController(title: header, message: messsage, preferredStyle: UIAlertControllerStyle.Alert)
